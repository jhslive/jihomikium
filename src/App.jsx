...
function App() {
  ...
  const [result, setResult] = useState(
    ((((maxPrice/askPrice)-(maxPrice/askPrice)*0.00015)*bidPrice) -(maxPrice/askPrice)*bidPrice*0.00015)-maxPrice / askPrice - 1) * 75 * 100).toFixed(2)
  );
  ...
  return (
    <div className="App">
      ...
      >
          ...
          >
            +{(Number(((((maxPrice/askPrice)-(maxPrice/askPrice)*0.00015)*bidPrice) -(maxPrice/askPrice)*bidPrice*0.00015)-maxPrice).toFixed(0)).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ',')}
          </div>
           <div
            style={{
              position: "relative",
